...
def video_metadata(path: Path) -> dict:
    ...
    if result.returncode:
        raise ValueError("ملف الفيديو تالف أو غير مكتمل. جرّب تنزيله مجددًا.")
    data = json.loads(result.stdout)
    video = next((s for s in data.get("streams", []) if s.get("codec_type") == "video"), None)
    if not video or not video.get("width") or not video.get("height"):
        raise ValueError("الملف لا يحتوي على فيديو صالح للتشغيل.")
    duration = float(data.get("format", {}).get("duration") or video.get("duration") or 0)
    if duration <= 0:
        raise ValueError("ملف الفيديو فارغ أو غير مكتمل.")
    return {"width": int(video["width"]), "height": int(video["height"]),
            "duration": max(1, round(duration))}


def normalize_video(path: Path) -> Path:
    """Encode actual video/audio content, not just rename the extension."""
    video_metadata(path)
    output = path.parent / "compatible" / "video.mp4"
    ...
    result = subprocess.run([
        ...
        "-pix_fmt", "yuv420p", "-tag:v", "avc1", "-threads", "2",
        "-c:a", "aac", "-b:a", "128k", "-ac", "2",
        "-movflags", "+faststart", str(output),
    ], stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, text=True)
    if result.returncode:
        logger.error("Video conversion failed: %s", result.stderr[-2000:])
        raise ValueError("تعذّر تجهيز فيديو صالح للتشغيل. جرّب تنزيله مجددًا أو أرسل رابطًا آخر.")
    video_metadata(output)
    return output


def prepare_video_parts(path: Path) -> list[Path]:
    """No duration cutoff: encode the whole large video into bounded MP4 parts."""
    path = normalize_video(path)
    if path.stat().st_size <= MAX_FILE_BYTES:
        return [path]
    parts_dir = path.parent / "parts"
    parts_dir.mkdir(exist_ok=True)
    # Capped bitrate plus short closed GOPs keeps each part below the upload cap.
    # A size check is still mandatory; never upload an oversized part.
    segment_seconds = 240
    while segment_seconds >= 1:
        for old_part in parts_dir.glob("part_*.mp4"):
            old_part.unlink()
        result = subprocess.run([
            "ffmpeg", "-nostdin", "-hide_banner", "-loglevel", "error", "-xerror", "-y",
            "-i", str(path), "-map", "0:v:0", "-map", "0:a:0?",
            "-vf", "scale=trunc(iw/2)*2:trunc(ih/2)*2",
            "-c:v", "libx264", "-preset", "veryfast", "-crf", "23",
            "-threads", "2", "-maxrate", "1000k", "-bufsize", "2000k", "-pix_fmt", "yuv420p",
            "-force_key_frames", "expr:gte(t,n_forced*2)", "-flags", "+cgop",
            "-c:a", "aac", "-b:a", "128k", "-ac", "2",
            "-f", "segment", "-segment_time", str(segment_seconds),
            "-reset_timestamps", "1", "-segment_format", "mp4",
            "-segment_format_options", "movflags=+faststart",
            str(parts_dir / "part_%06d.mp4"),
        ], stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, text=True)
        if result.returncode:
            logger.error("ffmpeg failed: %s", result.stderr[-2000:])
            raise ValueError("\u062a\u0639\u0630\u0651\u0631 \u062a\u062c\u0647\u064a\u0632 \u0623\u062c\u0632\u0627\u0621 \u0627\u0644\u0641\u064a\u062f\u064a\u0648. \u062c\u0631\u0651\u0628 \u0631\u0627\u0628\u0637\u064b\u0627 \u0622\u062e\u0631.")
        parts = sorted(parts_dir.glob("part_*.mp4"))
        if parts and all(0 < part.stat().st_size <= MAX_FILE_BYTES for part in parts):
            for part in parts:
                video_metadata(part)
            return parts
        segment_seconds //= 2
    raise ValueError("\u062a\u0639\u0630\u0651\u0631 \u062a\u0642\u0633\u064a\u0645 \u0627\u0644\u0641\u064a\u062f\u064a\u0648 \u0625\u0644\u0649 \u0623\u062c\u0632\u0627\u0621 \u0628\u0627\u0644\u062d\u062c\u0645 \u0627\u0644\u0645\u0646\u0627\u0633\u0628 \u0644\u0644\u0625\u0631\u0633\u0627\u0644.")
        ...
        async with DOWNLOAD_SLOTS:
            with tempfile.TemporaryDirectory(prefix="video_bot_") as temp_dir:
                ...
                if video_path.stat().st_size > MAX_FILE_BYTES:
                    await status.edit_text("\u0627\u0644\u0641\u064a\u062f\u064a\u0648 \u0643\u0628\u064a\u0631\u061b \u062c\u0627\u0631\u064a \u062a\u062c\u0647\u064a\u0632\u0647 \u0643\u0627\u0645\u0644\u064b\u0627 \u0639\u0644\u0649 \u0623\u062c\u0632\u0627\u0621. \u0642\u062f \u064a\u0633\u062a\u063a\u0631\u0642 \u0630\u0644\u0643 \u0628\u0639\u0636 \u0627\u0644\u0648\u0642\u062a\u2026")
                parts = await asyncio.to_thread(prepare_video_parts, video_path)
                for index, part in enumerate(parts, 1):
                    if len(parts) > 1:
                        await status.edit_text(f"\u062c\u0627\u0631\u064a \u0625\u0631\u0633\u0627\u0644 \u0627\u0644\u062c\u0632\u0621 {index} \u0645\u0646 {len(parts)}\u2026")
                    metadata = await asyncio.to_thread(video_metadata, part)
                    with part.open("rb") as video_file:
                        await update.message.reply_video(
                            video=video_file, supports_streaming=True,
                            **metadata, read_timeout=300, write_timeout=600,
                        )
                    if len(parts) > 1:
