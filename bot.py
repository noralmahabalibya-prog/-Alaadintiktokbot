            ...
            with tempfile.TemporaryDirectory(prefix="video_bot_") as temp_dir:
                ...
                for index, part in enumerate(parts, 1):
                    ...
                    with part.open("rb") as video_file:
                        send = (update.message.reply_video if part.suffix.lower() == ".mp4"
                                else update.message.reply_document)
                        media = ({"video": video_file, "supports_streaming": True}
                                 if part.suffix.lower() == ".mp4" else {"document": video_file})
                        await send(**media, read_timeout=300, write_timeout=600)
                    if len(parts) > 1:
